...
q_encoder = BERTEmbedder()  
q_encoder.to(device) 

p_encoder = BERTEmbedder()  
p_encoder.to(device)  
...
for epoch_i in tqdm(range(0, epochs), desc="Epochs", position=0, leave=True, total=epochs):
    ...
    with torch.no_grad(): 
        ...
        if avg_top_1 > best_recall: 
            ...
            torch.save(q_encoder.state_dict(), f"meanpooling_lawqa_bert_query_encoder_law_QA.pt") 
            torch.save(p_encoder.state_dict(), f"meanpooling_lawqa_bert_passage_encoder_law_QA.pt") 
